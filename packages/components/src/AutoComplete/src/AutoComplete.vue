  ...
  >
    ...
    >
      <s-input 
        ...
        ref="inputRef"
        :readonly="!isDropdownShow"
        @input="debouceOnFilter"
        @keydown="handleKeydown"
      >
        <template #suffix>
          <s-icon 
            v-if="states.mouseHover"
            icon="circle-xmark"
      ...
      <template #content>
        <div class="vk-select__loading" v-if="states.loading"><s-icon icon="spinner" spin/></div>
        <div class="vk-select__nodata" v-else-if="filteredOptions.length === 0">no matching data</div>
...
import '../style/style.css'
  ...
  const filteredPlaceholder = computed(() => {
    return (states.selectedOption && isDropdownShow.value) 
